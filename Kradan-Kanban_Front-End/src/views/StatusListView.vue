...
const closeEdit = (res) => {
  ...
  if (typeof res === "object") {
    ...
    toastStore.createToast("An error has occurred, the status does not exist", "danger")
};
      ...
      <div class="w-3/4 mx-auto mt-10 relative">
        <div class="float-right flex flex-row">
          <div :class="isOwner ? '' : 'lg:tooltip'" data-tip="You don't have a permission to Add a status">
            <button class="itbkk-button-add btn btn-square btn-outline w-16 float-left mr-1"
      ...
      <transition>
        <table v-if="!loading"
          ...
          <tbody>
            ...
            <tr v-if="status !== null" v-for="(status, index) in statusStore.status" :key="status.id"
              ...
              <td v-if="status.name !== 'No Status' && status.name !== 'Done'"
                ...
                <div :class="isOwner ? '' : 'lg:tooltip'" data-tip="You don't have a permission to Edit a status">
                  <button class="itbkk-button-edit btn m-2" @click="openEdit(status.id)" :disabled="!isOwner">
                    Edit
                  </button>
                </div>
