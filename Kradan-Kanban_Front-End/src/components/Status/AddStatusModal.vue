...
const editStatusTitleLength = computed(() => {
  return statusData.value?.name?.trim().length;
});
const editStatusDescriptionLength = computed(() => {
  return statusData.value?.description?.trim()?.length;
});

watch(statusData.value, () => {
  if (statusData.value.name.trim().length > 50) Errortext.value.name = "Status Name can't long more 50 character";
  else if (statusData.value.name.trim().length === 0) Errortext.value.name = "Status Name can't be empty";
  ...
  if (statusData.value.description.trim().length > 200) Errortext.value.description = "Status Description can't long more 200 character";
  else Errortext.value.description = '';
  canSave.value = Errortext.value.name === '' && Errortext.value.description === '';
});

async function fetchStatusData() {
  statusData.value.name = statusData.value.name.trim();
  statusData.value.description = statusData.value.description.trim();
  statusData.value.color = colorPicking.value.split('#')[1];
  ...
  <div class="h-[50rem] w-full bg-white rounded-md flex flex-col overflow-scroll">
    ...
    <div class="flex flex-col overflow-scroll p-2 grow" ref="content" @scroll="handelScroll" v-if="loading.length === 0">
      <div class="w-full flex flex-row">
        <label class="form-control w-full">
          ...
          <input v-model="statusData.name" type="text" placeholder="Type here" class="itbkk-title input input-bordered w-full bg-white dark:bg-base-300 dark:text-slate-400" />
          <div class="label">
            <!-- ? Error Text -->
        ...
        <div class="relative">
          ...
          <div v-if="descriptionTab === 'preview'">
            <div
              ...
              v-html="marked(statusData.description === null ? '' : statusData.description)"
            ></div>
          </div>
        </div>
      </label>
    ...
    <div class="sticky bottom-0 w-full z-50 gap-4 p-2" :class="isFooterSticky ? 'shadow-top' : ''">
      <div class="float-right flex flex-row-reverse gap-3">
        ...
        <button class="itbkk-button-confirm btn btn-outline btn-success basis-1/6" :disabled="!canSave" :class="!canSave ? 'disabled' : ''" @click="fetchStatusData()">
